...
fn main() {
    ...
    let mint_path = "/home/astrobyte/personalProjects/segp/rebu/token-info/.config/solana/mint_keypair.json";

    let mint = get_pubkey_from_file(&mint_path);
    println!("Mint account: {:?}", &mint);

    let user1 = Keypair::new();
    println!("User account: {:?}", user1_pubkey);

    let user2 = Keypair::new();
    println!("User account: {:?}", user2_pubkey);
