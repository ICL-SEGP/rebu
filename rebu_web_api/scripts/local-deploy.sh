...
if [ -z "$AWS_ACCESS_KEY_ID" ] || [ -z "$AWS_SECRET_ACCESS_KEY" ] || [ -z "$AWS_REGION" ]; then
  echo "Error: Please set the AWS_REGION, AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY."
  exit 1
fi
...
IMAGE=${1:-"ghcr.io/icl-segp/rebu/rebu_web_api:latest"}
INSTANCE_TAG_NAME="web-server-segp"
...
source "$CURRENT_SCRIPT_DIRECTORY/decrypt.sh"
...
# # run the two below if there are new changes
