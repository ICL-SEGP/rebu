...
import { affiliateGetUsers } from "@/lib/api/affiliate";
...
export default function UsersPage() {
  ...
  // Handle role filter
  const handleFilterRole = (role: Role) => {
    setRoleFilter(role);
    filterUsers(searchQuery, role);
  };

  // Filter users based on search and role
  const filterUsers = (search: string, role: Role) => {
    let filtered = [...users];
  ...
  const updateUser = async (selectedUser) => {
    ...
    const res = await fetch(
      `${API_BASE_URL}/api/affiliate/users/${selectedUser.id}`,
      {
        method: "PUT",
        headers: {
  ...
  const handleTokenGive = (user: string) => {
    ...
    toast({
      title: "Tokens Given!",
      description: numericValue + " tokens given.",
    });
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex gap-4">
        ...
        <Select
          value={roleFilter}
          onValueChange={(value) => handleFilterRole(value as Role)}
        >
          <SelectTrigger className="w-40">
            <SelectValue placeholder="Filter by role" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={Role.ALL}>All</SelectItem>
            <SelectItem value={Role.AFFILIATE}>Affiliate</SelectItem>
            <SelectItem value={Role.USER}>Regular</SelectItem>
          </SelectContent>
        </Select>
      </div>
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(value) =>
                        setUsers((prev) =>
                          prev.map((u) =>
                            u.id === user.id ? { ...u, role: value as Role } : u
                          )
                        )
                      }
                    >
