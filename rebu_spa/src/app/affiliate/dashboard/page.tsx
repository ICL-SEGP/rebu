...
import SalesDashboard from "@/components/owned/monthlyBreakdown";
import { stat } from "fs";
import {
  Chart as ChartJS,
  BarElement,
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (usersList) {
      ...
      usersList.sort((a, b) => b.dateJoined.getTime() - a.dateJoined.getTime());
  ...
  const { data: statList } = useQuery({
    queryKey: ["stats"],
    queryFn: () => getAffiliateStats(session!.accessToken),
  });

  function processMonthlyBreakdown(stats: StatsObject): MonthlyBreakdownItem[] {
    return Object.entries(stats.monthly_breakdown).map(([key, value]) => {
      return {
        ...
        tokensRefunded: parseFloat(value.total_cancelled_tokens).toFixed(2),
      };
    });
  }
  ...
  useEffect(() => {
    if (statList) {
      ...
      setStats(statList);
      setMonthlyBreakdown(processMonthlyBreakdown(statList));
    }
  }, [statList]);
  ...
  return (
    <div className="space-y-8 p-6">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-semibold flex items-center gap-1">
              ...
              {parseFloat(stats.balances ?? 0).toFixed(2)}
            </p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <CardTitle>Your Newest Users</CardTitle>
