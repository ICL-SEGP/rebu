...
import { EyeOff, UserIcon, EyeIcon, ClipboardCopy } from "lucide-react";
...
export default function UserProfile() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await updateAffiliateProfile(session.accessToken, formData);
      setEditing(false);
  ...
  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(
        generateReferralLink(session?.user.referralCode)
      );
