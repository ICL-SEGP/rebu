...
import NewOrderForm from "@/components/owned/new-order-form";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import { Offer, Order, OrderStatus, Role, toOfferIds } from "@/types/types";
...
export default function OrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      ...
      const dateSearch = dateString.includes(searchQuery.toLowerCase().trim());
    ...
    try {
      ...
      setOrders((prevOrders: Order[]) =>
        prevOrders.map((order) =>
          order.id === tempOrder.id ? updatedOrder : order
        )
      );
            ...
            {orders?.length > 0 ? (
              orders.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell>{order.totalRebateAmount}</TableCell>
