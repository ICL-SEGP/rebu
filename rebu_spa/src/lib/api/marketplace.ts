...
import { Product, Review, Purchase, Category, toProduct } from "@/types/types";
...
export function createPurchase(
  ...
  return fetch(`${API_BASE_URL}/purchase`, {
    ...
    body: JSON.stringify({
      purchase: humps.decamelizeKeys(purchase),
    }),
  })
    .then((response) => {
      if (!response.ok) {
...
export async function createProduct(
  ...
  const response = await fetch(`${API_BASE_URL}/products`, {
    ...
    body: JSON.stringify({
      product: humps.decamelizeKeys(product),
    }),
  });
...
export function getPurchases(token: string) {
  return fetch(`${API_BASE_URL}/purchase/history`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    ...
    .then((purchases) => humps.camelizeKeys(purchases));
}
...
export function deleteReview(
  ...
  return fetch(`${API_BASE_URL}/purchase/reviews/${reviewId}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    ...
    if (!response.ok) {
      return response.text().then((errorText) => {
        throw new Error(
          `Failed to delete review: ${response.statusText} - ${errorText}`
        );
      });
    }
    return response.json();
...
export async function getReviewsForProduct(
  ...
  const response = await fetch(
    `${API_BASE_URL}/purchase/reviews/${productId}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch reviews for ${productId}: ${response.statusText}`
    );
  }
  return response.json();
}
...
export function saveReview(token: string, review: Review): Promise<Review> {
  return fetch(`${API_BASE_URL}/product/reviews/create`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ review: humps.decamelizeKeys(review) }),
  }).then((response) => {
    if (!response.ok) {
      return response.text().then((errorText) => {
        throw new Error(
          `Failed to save review: ${response.statusText} - ${errorText}`
        );
      });
    }
    return response.json();
