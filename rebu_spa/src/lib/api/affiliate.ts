...
export function updateAffiliateProfile(
  ...
  return fetch(`${API_BASE_URL}/affiliate`, {
    ...
    body: JSON.stringify({ affiliate: humps.decamelizeKeys(updatedAffiliate) }),
  })
    .then((response) => {
      if (!response.ok) {
...
export function affiliateBlockUser(token: string, id: string): Promise<any> {
  return fetch(`${API_BASE_URL}/affiliate/users/${id}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    ...
    if (!response.ok) {
      throw new Error(`Failed to block user ${id}: ${response.statusText}`);
    }
